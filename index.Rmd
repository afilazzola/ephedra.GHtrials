...
plot1 <- ggplot(recruit %>% filter(Density != 0), aes(x=Density, y= brome.begin)) + geom_jitter(height=0.5, width=0.5) + xlim(1,21) + ylim(0,20) + geom_smooth(method="glm", method.args = list(family = "quasipoisson"), fullrange=T, color="#56B4E9") + theme_Publication() + xlab(expression("Number of "*paste(italic("B. madritensis"))*" seeds")) + ylab("Number of emerged individuals")

plot2 <- ggplot(recruit %>% filter(Density != 0), aes(as.factor(Density), y=brome.prop.begin))  + geom_boxplot(fill="#56B4E9") + theme_Publication() + xlab(expression("Number of "*paste(italic("B. madritensis"))*" seeds")) + ylab(expression("Proportion of emerged"*paste(italic("B. madritensis"))*" individuals")) + facet_grid(~treat)
