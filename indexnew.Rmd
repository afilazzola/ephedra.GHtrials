...
ggplot(outall %>% filter(comparison %in% c("emergence", "survival")), aes(x=treatment, y=estimate)) + geom_bar(stat="identity") + geom_errorbar(aes(ymin=estimate-SE*1.96, ymax=estimate+SE*1.96), width=0) + facet_grid(comparison~1) + theme_Publication() + ylab("Estimated marginal means")
...
ggplot(outall %>% filter(comparison %in% c("ABG", "BWG","bromeBio")), aes(x=treatment, y=estimate)) + geom_bar(stat="identity") + geom_errorbar(aes(ymin=estimate-SE*1.96, ymax=estimate+SE*1.96), width=0) + facet_grid(biomass~1,  scales= "free_y") + theme_Publication() + ylab("Estimated marginal means")
...
predEmerge <- predict(m1, newdata=data.frame(Density = 0:20), type="response", se.fit=T)
predData <- data.frame(Density = 0:20, fit=predEmerge$fit, error=predEmerge$se.fit)


ggplot(predData, aes(x=Density, y=fit)) + geom_line()

ggplot( recruit, aes(x=Density, y=survival)) +
  geom_point() +
